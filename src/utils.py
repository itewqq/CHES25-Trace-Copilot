...
def compile_target(base_addr, target="make_target.S", mcpu="cortex-m4", add_options=""):
    as_cmd = f"arm-none-eabi-as -mthumb -mimplicit-it=thumb -mcpu={mcpu} {add_options} {target} -o as.out"
...
def patch_bytes_img(file_path: str, img_offset: int, target_bytes: bytes):
    try:
        ...
        print(f"\t[+] patched {hex(img_offset)}: {len(target_bytes)}")
    except Exception as e:
        print(e)
        # print(f"\t[x] patched {hex(img_offset)} error!")

def get_new_bytes_offsets(asm_template, elf_path="a.out"):
    result = b""
    # new_hard_fault_handler_offset = 0
    with open(elf_path, "rb") as f:
        all_bytes = f.read()
        elf = ELF(elf_path)
        if 'got_PC_location' in asm_template:
            print(f"[*] breakpoint address should be: {hex(elf.symbols['got_PC_location'])}")
            with open("hwb.py", "a") as f:
                f.write(f"hwb = {hex(elf.symbols['got_PC_location'])}\n")
        new_reset_handler_va = elf.symbols["_start"]
        new_hard_fault_handler_va = elf.symbols["new_hard_fault_handler"]
        text_seg = elf.get_section_by_name('.text')
        text_offset, text_va, text_size = text_seg.header.sh_offset, text_seg.header.sh_addr, text_seg.header.sh_size
        
        result = all_bytes[text_offset:text_offset+text_size]
    return result, new_reset_handler_va, new_hard_fault_handler_va

def patch_binary_handlers(file_path_patched: str, hf_handler_new_offset: int, reset_handler_new_offset):
    ...
    patch_bytes_img(file_path_patched, reset_handler_vtable_img_offset,
                    (reset_handler_new_offset + 1).to_bytes(4, "little"))
    patch_bytes_img(file_path_patched, hard_fault_vtable_offset,
                    (hf_handler_new_offset + 1).to_bytes(4, "little"))
